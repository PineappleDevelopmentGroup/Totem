...
class EntityDamageListener : Listener {
    ...
    private val doNotRevive = mutableSetOf<UUID>()
    ...
    fun onEntityTakeDamage(event: EntityDamageEvent) {
        ...
        val firstTotem =
            TotemInventoryUtils.getEquipmentSlotsWithTarget(Material.TOTEM_OF_UNDYING, equipment).stream().filter {
        ...
        firstTotem.ifPresentOrElse({
            ...
            doNotRevive.add(entity.uniqueId)
        })
    }

    @EventHandler
    fun onResurrect(event: EntityResurrectEvent) {
        if (doNotRevive.contains(event.entity.uniqueId)) {
            doNotRevive.remove(event.entity.uniqueId)
            event.isCancelled = true
        }
    }
